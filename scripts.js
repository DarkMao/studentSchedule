...
$.getJSON('data.json',function(data){
   ...
   $('#studentName').keypress(function(event){
    ...
    //         // $('#studentName').data("autocomplete").options.select(event, {item: item});
    //         console.log(item);
    //         var studentNameGrade = document.getElementById('studentName').value;
    //         selected.push(data.filter(entry => entry.id === studentNameGrade)[0]);
    //         // event.preventDefault();
    //         $('#studentName').val("");
    //         console.log(selected);
    //     }
    ...
    $("#studentName").autocomplete({source: namesGrades,// fixed so that auto focuses first element on pressing enter
    autoFocus: true,
...
var times = [
    {per: "A", start: [8,25], end: [9,10]},
    {per: "B", start: [9,11], end: [9,57]},
    {per: "C", start: [12,19], end: [1,05]},
    {per: "D", start: [1,06], end: [1,52]},
    {per: "E", start: [1,53], end: [2,39]},
    {per: "F", start: [9,58], end: [10,44]},
    {per: "G", start: [11,31], end:[12,18]}
];
var d = new Date();
var day = d.getDay()-1;
